...
const WorkDetail: React.FC = () => {
    ...
    // 3 FIELDS LOGIC
    const [expTotalAgreed, setExpTotalAgreed] = useState(''); // Field 1: Planned
    const [alreadyPaidLocked, setAlreadyPaidLocked] = useState(0); // Field 2: Already Paid (Locked)
    const [payNowAmount, setPayNowAmount] = useState(''); // Field 3: Pay Now (Editable)
    ...
    const openAddExpense = () => {
        ...
        // 2. Reset 3-Field Logic
        setExpTotalAgreed('');      // Field 1: Plan
        setAlreadyPaidLocked(0);    // Field 2: Locked Paid (New = 0)
        setPayNowAmount('');        // Field 3: Pay Now
    };
    ...
    const openEditExpense = (expense: Expense) => {
        ...
        // --- 3 FIELDS LOGIC IMPLEMENTATION ---
        // Field 1: Planned Amount (Total Agreed)
        setExpTotalAgreed(expense.totalAgreed ? String(expense.totalAgreed) : '');
        
        // Field 2: Already Paid (Locked) - Is the CURRENT amount saved in DB
        setAlreadyPaidLocked(expense.amount || 0);
        
        // Field 3: Pay Now - Starts empty or 0, user enters increment
        setPayNowAmount('');
    };
    ...
    const handleSaveExpense = async (e: React.FormEvent) => {
        ...
        // Logic: New Total = (Already Paid Locked) + (Pay Now Input)
        const payNow = payNowAmount ? Number(payNowAmount) : 0;
        const cumulativeAmount = alreadyPaidLocked + payNow;
        ...
        if (expenseModal.mode === 'ADD') {
            await dbService.addExpense({
                ...
                workId: work.id,
                description: expDesc,
                amount: cumulativeAmount, // Save total accumulated
            ...
            if (existing) {
                await dbService.updateExpense({
                    ...existing,
                    description: expDesc,
                    amount: cumulativeAmount, // Update with new total
                    date: new Date(expDate).toISOString(),
                    category: expCategory,
                    stepId: finalStepId,
                    totalAgreed: finalTotalAgreed
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm p-6 shadow-2xl">
                        ...
                        <form onSubmit={handleSaveExpense} className="space-y-4">
                            ...
                            {/* 3 FIELD LOGIC: PLANNED, PAID, PAY NOW */}
                            <div className="space-y-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                                
                                {/* Field 1: PLANNED (Editable) */}
                                <div>
                                    <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1">Valor Planejado (Total)</label>
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            disabled={expenseModal.isAutoGenerated}
                                            className="w-full pl-10 p-2.5 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-600 font-bold text-sm text-slate-700 dark:text-slate-300" 
                                        />
                                    </div>
                                </div>

                                {/* Field 2: ALREADY PAID (Locked/Read-only) */}
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex justify-between">
                                        <span>Já Pago (Histórico)</span>
                                    ...
                                    <div className="relative opacity-70">
                                        ...
                                        <input 
                                            type="text" 
                                            value={alreadyPaidLocked.toLocaleString('pt-BR', {minimumFractionDigits: 2})}
                                            disabled
                                            className="w-full pl-10 p-2.5 bg-slate-100 dark:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600 font-bold text-sm text-slate-500 cursor-not-allowed" 
                                        />
                                    </div>
                                </div>

                                {/* Field 3: PAY NOW (Editable, Highlighted) */}
                                <div>
                                    <label className="text-[10px] font-black text-secondary uppercase tracking-widest ml-1 flex items-center gap-2">
                                        <i className="fa-solid fa-money-bill-wave"></i> Valor a Pagar Agora
                                    </label>
                                    <div className="relative">
                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-secondary font-bold">R$</span>
                                        <input 
                                            type="number" 
                                            placeholder="0.00" 
                                            value={payNowAmount} 
                                            ...
                                            disabled={expenseModal.isAutoGenerated}
                                            autoFocus
                                            className="w-full pl-10 p-3 bg-white dark:bg-slate-900 rounded-lg border-2 border-secondary/30 focus:border-secondary font-black text-lg text-primary dark:text-white shadow-sm transition-all" 
                                        />
                                    </div>
                                </div>
