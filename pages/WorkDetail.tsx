...
const SectionHeader: React.FC<{ title: string; subtitle: string }> = ({ title, subtitle }) => (
  <div className="mb-6 print:mb-2">
    ...
    <div className="h-1 w-10 bg-secondary rounded-full mt-3 print:hidden" />
  </div>
);
...
// 1. CONTACTS VIEW
const ContactsView: React.FC<{ mode: 'TEAM' | 'SUPPLIERS'; onBack: () => void }> = ({
  mode,
  onBack
}) => {
  const { user } = useAuth();
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4">
      <button
        onClick={onBack}
        className="mb-4 text-sm font-bold text-slate-400 hover:text-primary"
      >
      ...
      <div className="space-y-3">
        {items.map(item => (
          <div
            key={item.id}
            className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-800 flex justify-between items-center"
          >
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center text-white ${
                  mode === 'TEAM' ? 'bg-blue-500' : 'bg-indigo-500'
                }`}
              >
                <i
                  className={`fa-solid ${
                    mode === 'TEAM' ? 'fa-helmet-safety' : 'fa-truck'
                  }`}
                />
              </div>
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {(item as any).role || (item as any).category}
                </p>
              </div>
            </div>
      ...
      <button
        ...
        className="mt-6 w-full py-3 bg-primary text-white rounded-xl font-bold shadow-lg"
      >
        <i className="fa-solid fa-plus mr-2" /> Adicionar
      </button>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
            ...
            <form onSubmit={handleAdd} className="space-y-3">
              <input
                ...
                className="w-full p-3 rounded-xl border dark:border-slate-700 dark:bg-slate-800 dark:text-white outline-none"
                required
              />
              <select
                value={newRole}
...
// 2. PHOTOS VIEW
const PhotosView: React.FC<{ workId: string; onBack: () => void }> = ({
  workId,
  onBack
}) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4">
        ...
        {photos.map(p => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <button
                onClick={async () => {
                  await dbService.deletePhoto(p.id);
                  loadPhotos();
                }}
      ...
      {photos.length === 0 && (
        <p className="text-center text-slate-400 py-10">Nenhuma foto.</p>
      )}
    </div>
  );
};
...
// 3. FILES VIEW
const FilesView: React.FC<{ workId: string; onBack: () => void }> = ({
  workId,
  onBack
}) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4">
        ...
        {files.map(f => (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-sm text-primary dark:text-white truncate max-w-[150px]">
                  {f.name}
                </h4>
                <p className="text-xs text-slate-500">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={async () => {
                  await dbService.deleteFile(f.id);
                  loadFiles();
                }}
      ...
      {files.length === 0 && (
        <p className="text-center text-slate-400 py-10">Nenhum arquivo.</p>
      )}
    </div>
  );
};
...
// 4. REPORTS VIEW
const ReportsView: React.FC<{ workId: string; onBack: () => void }> = ({
  workId,
  onBack
}) => {
  const [activeTab, setActiveTab] = useState<'FINANCIAL' | 'MATERIALS' | 'STEPS'>(
    'FINANCIAL'
  );
  ...
  const purchasedMaterials = materials.filter(
    m => m.purchasedQty >= m.plannedQty
  ).length;
  ...
  const materialChartData = [
    ...
    {
      name: 'Pendente',
      value: materials.length - purchasedMaterials,
      fill: '#E2E8F0'
    }
  ];
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 bg-white dark:bg-slate-950 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6 print:hidden">
        <button
          onClick={onBack}
          className="text-sm font-bold text-slate-400 hover:text-primary flex items-center gap-2"
        >
          <i className="fa-solid fa-arrow-left" /> Voltar
        </button>
        <div className="flex gap-2">
          <button
            onClick={handlePrint}
            className="bg-primary text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg flex items-center gap-2"
          >
      ...
      <SectionHeader
        ...
        subtitle="Analise cada detalhe da sua obra."
      />
          ...
          <div className="space-y-4">
            {Object.keys(groupedMaterials)
              ...
              .map(cat => (
                <div
                  key={cat}
                  className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 break-inside-avoid"
                >
                  <h4 className="font-bold text-primary dark:text-white mb-4 border-b border-slate-100 dark:border-slate-800 pb-2">
                    {cat}
                  </h4>
        ...
        <div className="space-y-6 animate-in fade-in">
          <div className="flex gap-4 mb-4 overflow-x-auto pb-2">
            ...
            <div className="flex-1 min-w-[120px] bg-red-50 dark:bg-red-900/20 p-4 rounded-xl border border-red-100 dark:border-red-900/30 text-center">
              ...
              <p className="text-xs font-bold text-red-700 dark:text-red-300 uppercase">
                Atrasadas
              </p>
            </div>
...
const CalculatorView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  const [mode, setMode] = useState<'MENU' | 'FLOOR' | 'WALL' | 'PAINT' | 'ESTIMATOR'>(
    'MENU'
  );
  ...
  const ResultCard: React.FC<{ label: string; value: string; sub?: string }> = ({
    label,
    value,
    sub
  }) => (
      ...
      return (
        <div className="animate-in fade-in slide-in-from-right-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            <ResultCard label="Piso" value={`${res.tiles} m²`} sub="+10% Perda" />
            <ResultCard label="Argamassa" value={`${res.mortar} sc`} sub="20kg" />
            <ResultCard label="Rejunte" value={`${res.grout} kg`} />
          </div>
        </div>
      );
    }
        ...
        <div className="animate-in fade-in slide-in-from-right-4">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <input
                type="number"
                className="w-full text-2xl font-bold bg-transparent border-b border-slate-200 dark:border-slate-700 dark:text-white outline-none"
          ...
          <div className="grid grid-cols-1 gap-4">
            <ResultCard label="Tijolos" value={`${res.bricks} un`} />
            <ResultCard label="Cimento" value={`${res.cement} sc`} />
            <ResultCard label="Areia" value={`${res.sand} m³`} />
          </div>
        </div>
      );
    }
      ...
      return (
        <div className="animate-in fade-in slide-in-from-right-4">
          ...
          <div className="mb-6 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
            ...
            <input
              type="number"
              className="w-full text-3xl font-bold bg-transparent outline-none text-primary dark:text-white border-b border-slate-200 dark:border-slate-700 pb-2 mt-2"
              placeholder="0"
              onChange={e => setArea(Number(e.target.value))}
            />
          </div>
          <div className="grid grid-cols-1 gap-4">
            <ResultCard label="Tinta 18L" value={`${res.cans18} un`} />
            <ResultCard label="Massa" value={`${res.spackle} lt`} />
            <ResultCard label="Selador" value={`${res.sealer} lt`} />
          </div>
        </div>
      );
    }
        ...
        <div className="animate-in fade-in slide-in-from-right-4">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <input
                type="number"
                className="w-full text-2xl font-bold bg-transparent border-b outline-none dark:text-white"
          ...
          <div className="space-y-4">
            <ResultCard
              ...
              value={`${res.outlets + res.switches} un`}
            />
            <ResultCard
              label="Pontos Hidráulicos"
              value={`${res.toilets + res.sinks} un`}
            />
          </div>
        </div>
      );
    ...
    return (
      <div className="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4">
        {[
          ...
          >
            <div
              className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white mb-3 shadow-lg ${item.color}`}
            >
              <i className={`fa-solid ${item.icon} text-2xl`} />
            </div>
            <span className="font-bold text-slate-700 dark:text-slate-300 group-hover:text-primary dark:group-hover:text-white">
              {item.label}
            </span>
          </button>
        ))}
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4">
      <button
        ...
        className="mb-4 text-sm font-bold text-slate-400 hover:text-primary flex items-center gap-2"
      >
        <i className="fa-solid fa-arrow-left" /> Voltar
      </button>
    ...
    return (
      <div className="animate-in fade-in slide-in-from-right-4 h-full flex flex-col">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          >
            <i className="fa-solid fa-download" /> Baixar .doc
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        {CONTRACT_TEMPLATES.map(ct => (
          <button
            ...
            className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 hover:border-secondary transition-all text-left shadow-sm group"
          >
            <div className="flex items-start gap-4">
              <div className="w-12 h-12 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 flex items-center justify-center text-xl group-hover:scale-110 transition-transform">
                <i className="fa-solid fa-file-contract" />
              </div>
              <div>
                <h4 className="font-bold text-primary dark:text-white mb-1 group-hover:text-secondary transition-colors">
                  {ct.title}
                </h4>
...
const ChecklistsView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4">
      <button
        onClick={onBack}
        className="mb-4 text-sm font-bold text-slate-400 hover:text-primary flex items-center gap-2 print:hidden"
      >
        <i className="fa-solid fa-arrow-left" /> Voltar
      </button>
      ...
      <div className="flex justify-between items-center mb-6">
        <SectionHeader
          title="Checklists Anti-Erro"
          subtitle="O que verificar para evitar prejuízo."
        />
        <button
          onClick={() => window.print()}
          className="bg-slate-100 dark:bg-slate-800 text-slate-500 w-10 h-10 rounded-xl flex items-center justify-center print:hidden"
        >
      ...
      <div className="space-y-4">
        {STANDARD_CHECKLISTS.map((list, idx) => {
          ...
          return (
            <div
              key={idx}
              className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden print:border-black print:break-inside-avoid"
            >
              <button
                onClick={() =>
                  setOpenCategory(isOpen ? null : list.category)
                }
                  ...
                  <div className="space-y-4 mt-4">
                    {list.items.map((item, i) => (
                      ...
                      >
                        ...
                        />
                        <span className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed group-hover:text-primary dark:group-hover:text-white transition-colors">
                          {item}
                        </span>
                      </label>
                    ))}
...
const MoreMenuTab: React.FC<{ workId: string }> = ({ workId }) => {
  ...
  if (activeSection === 'TEAM')
    return <ContactsView mode="TEAM" onBack={() => setActiveSection(null)} />;
  if (activeSection === 'SUPPLIERS')
    return (
      <ContactsView mode="SUPPLIERS" onBack={() => setActiveSection(null)} />
    );
  if (activeSection === 'PHOTOS')
    return <PhotosView workId={workId} onBack={() => setActiveSection(null)} />;
  if (activeSection === 'FILES')
    return <FilesView workId={workId} onBack={() => setActiveSection(null)} />;
  if (activeSection === 'REPORTS')
    return (
      <ReportsView workId={workId} onBack={() => setActiveSection(null)} />
    );
  if (activeSection === 'CALC')
    return <CalculatorView onBack={() => setActiveSection(null)} />;
  if (activeSection === 'CONTRACTS')
    return <ContractsView onBack={() => setActiveSection(null)} />;
  if (activeSection === 'CHECKLISTS')
    return <ChecklistsView onBack={() => setActiveSection(null)} />;
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
          <div className="w-20 h-20 rounded-full bg-secondary/10 flex items-center justify-center mb-4">
            <i className="fa-solid fa-robot text-4xl text-secondary" />
          </div>
          <h3 className="text-xl font-bold text-primary dark:text-white mb-2">
            IA do Zé da Obra
          </h3>
          <p className="text-slate-500 mb-6">
            Seu assistente está disponível no ícone de robô no topo da tela.
          </p>
        </div>
      </div>
    );
    ...
    <div className="animate-in fade-in duration-500 pb-24">
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {sections.map(s => (
          ...
          >
            <div
              className={`w-12 h-12 rounded-xl flex items-center justify-center text-white mb-2 shadow-lg ${s.color}`}
            >
      ...
      >
        {!isLifetime && (
          <div className="absolute inset-0 bg-white/60 dark:bg-black/60 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center text-center p-6">
            <i className="fa-solid fa-lock text-3xl text-slate-400 mb-3" />
            <h3 className="font-bold text-primary dark:text-white mb-1">
              Bônus Exclusivo
            </h3>
            <p className="text-xs text-slate-500 mb-4">
              Disponível no Plano Vitalício
            </p>
            <button
              onClick={() => (window.location.hash = '#/settings')}
              className="bg-premium text-white px-6 py-2 rounded-xl font-bold shadow-lg shadow-purple-500/20 text-sm"
            >
              Liberar Acesso
            </button>
          </div>
        ...
        <div className="relative z-0">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3
                className={`font-bold ${
                  isLifetime ? 'text-white' : 'text-slate-700 dark:text-slate-200'
                }`}
              >
                Ferramentas Premium
              </h3>
              <p
                className={`text-xs ${
                  isLifetime ? 'text-slate-400' : 'text-slate-500'
                }`}
              >
          ...
          <div className="grid grid-cols-2 gap-3">
            {bonusFeatures.map(f => (
              <button
                key={f.id}
                onClick={() => {
                  if (isLifetime) setActiveSection(f.id);
                }}
              ...
              >
                <i
                  className={`fa-solid ${f.icon} text-xl mb-2 ${
                    isLifetime ? 'text-secondary' : 'text-slate-400'
                  }`}
                />
                <h4
                  className={`font-bold text-sm mb-0.5 ${
                    isLifetime ? 'text-white' : 'text-slate-600 dark:text-slate-300'
                  }`}
                >
                  {f.label}
                </h4>
                <p
                  className={`text-[10px] leading-tight ${
                    isLifetime ? 'text-slate-400' : 'text-slate-400'
                  }`}
                >
                  {f.desc}
                </p>
...
const OverviewTab: React.FC<{
  ...
  onGoToSteps: () => void;
}> = ({ work, stats, onGoToSteps }) => {
  const budgetUsage =
    work.budgetPlanned > 0 ? (stats.totalSpent / work.budgetPlanned) * 100 : 0;
    ...
    <div className="animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden group">
          <h3 className="absolute top-6 left-6 text-xs text-slate-400 uppercase font-bold tracking-widest">
            Avanço Físico
          </h3>
          ...
          <div className="relative z-10">
            ...
            <div className="mb-8">
              ...
              <p className="text-sm text-slate-400 font-medium">
                de R$ {work.budgetPlanned.toLocaleString('pt-BR')} planejado
              </p>
            </div>
            ...
            <div className="w-full bg-black/30 rounded-full h-2 mb-2 overflow-hidden backdrop-blur-sm">
              <div
                className={`h-full rounded-full transition-all duration-1000 ${
                  budgetUsage > 100 ? 'bg-red-500' : 'bg-secondary'
                }`}
      ...
      <button
        ...
        className={`w-full bg-white dark:bg-slate-900 p-6 rounded-2xl border transition-all flex items-center justify-between group ${
          stats.delayedSteps > 0
            ? 'border-red-200 dark:border-red-900/30 shadow-lg shadow-red-500/5 hover:-translate-y-1'
            : 'border-slate-100 dark:border-slate-800 hover:border-success/30'
        }`}
      >
        <div className="flex items-center gap-4">
          ...
          >
            <i
              className={`fa-solid ${
                stats.delayedSteps > 0 ? 'fa-clock' : 'fa-check-circle'
              }`}
            />
          </div>
          <div>
            ...
            >
              {stats.delayedSteps > 0
                ? `${stats.delayedSteps} Etapas Atrasadas`
                : 'Cronograma em dia'}
            </h3>
        ...
        {stats.delayedSteps > 0 && (
          <i className="fa-solid fa-chevron-right text-slate-300 group-hover:text-red-500 transition-colors" />
        )}
      </button>
    </div>
  );
...
// --- TABS (STEPS) ---
const StepsTab: React.FC<{ workId: string; refreshWork: () => void }> = ({
  workId,
  refreshWork
}) => {
  const [steps, setSteps] = useState<Step[]>([]);
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="flex items-center justify-between mb-8">
        <SectionHeader
          title="Cronograma"
          subtitle="Toque para mudar o status."
        />
        <button
      ...
      <div className="space-y-4">
        {steps.map((step, idx) => {
          ...
          const isLate = !isComplete && new Date() > new Date(step.endDate);
          ...
          return (
            <div
              ...
              className={`group relative p-5 rounded-3xl border transition-all duration-300 ${
                isComplete
                  ...
                  : isInProgress
                  ? 'bg-white dark:bg-slate-800 border-secondary/30 ring-1 ring-secondary/20 shadow-lg shadow-secondary/5'
                  : isLate
            ...
            >
              {idx < steps.length - 1 && (
                <div className="absolute left-9 bottom-[-20px] top-[60px] w-0.5 bg-slate-100 dark:bg-slate-800 z-0" />
              )}
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                      <i className="fa-solid fa-pen text-slate-300 hover:text-secondary" />
                    </div>
                  </div>
                  ...
                  <div className="flex items-center flex-wrap gap-3 text-xs font-medium">
                    ...
                    {isLate && (
                      <span className="text-red-600 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded-md uppercase tracking-wide font-bold">
                        Atrasado
                      </span>
                    )}
                    {isInProgress && (
                      <span className="text-secondary bg-orange-50 dark:bg-orange-900/20 px-2 py-0.5 rounded-md uppercase tracking-wide font-bold">
                        Em Andamento
                      </span>
                    )}
                  </div>
            ...
            <form onSubmit={handleCreateStep} className="space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-4 rounded-xl font-bold text-slate-500 hover:bg-slate-50"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="flex-1 py-4 rounded-xl bg-primary text-white font-bold hover:bg-slate-800 shadow-lg"
                >
                  Salvar
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-primary/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm p-8 shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                onClick={() => handleDeleteClick(editingStep.id)}
                className="w-8 h-8 rounded-full bg-red-50 text-red-500 flex items-center justify-center hover:bg-red-100"
              >
            ...
            <form onSubmit={handleUpdateStep} className="space-y-5">
              <input
                className="w-full px-4 py-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-primary dark:text-white font-bold text-lg outline-none"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="date"
                    className="w-full px-3 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-sm outline-none"
...
// --- TABS (MATERIALS) ---
const MaterialsTab: React.FC<{ workId: string; onUpdate: () => void }> = ({
  workId,
  onUpdate
}) => {
  const [materials, setMaterials] = useState<Material[]>([]);
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-secondary w-12 h-12 rounded-2xl flex items-center justify-center transition-all"
          >
      ...
      {sortedCategories.map(cat => (
        <div key={cat} className="mb-8 last:mb-0">
          <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
            <i className="fa-solid fa-layer-group text-secondary" /> {cat}
          </h3>
          <div className="space-y-3">
            {groupedMaterials[cat].map(m => (
              <div
                    ...
                    <div className="h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${Math.min(
                            100,
                            (m.purchasedQty / m.plannedQty) * 100
                          )}%`
                        }}
                      />
                    </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm p-6 shadow-2xl h-[500px] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-2">
              {FULL_MATERIAL_PACKAGES.map(pkg => (
                ...
                >
                  <h4 className="font-bold text-primary dark:text-white group-hover:text-secondary">
                    {pkg.category}
                  </h4>
...
const ExpensesTab: React.FC<{ workId: string; onUpdate: () => void }> = ({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingId)
      await dbService.updateExpense({ ...(payload as any), id: editingId });
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="sticky top-0 z-30 bg-surface/90 dark:bg-slate-950/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 py-4 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-primary dark:text-white truncate max-w-[200px]">
            {work.name}
          </h1>
        </div>
        <button
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-6">
        {activeTab === 'overview' && (
          <OverviewTab
            work={work}
            stats={stats}
            onGoToSteps={() => setActiveTab('steps')}
          />
        )}
        {activeTab === 'steps' && (
        ...
        {activeTab === 'materials' && (
          <MaterialsTab workId={work.id} onUpdate={loadWork} />
        )}
        {activeTab === 'expenses' && (
          <ExpensesTab workId={work.id} onUpdate={loadWork} />
        )}
          ...
          >
            <div
              className={`w-10 h-10 rounded-2xl flex items-center justify-center text-lg transition-all ${
                activeTab === tab.id
                  ? 'bg-secondary text-white shadow-lg shadow-orange-500/30'
                  : ''
              }`}
            >
      ...
      {showAiChat && (
        <div className="fixed inset-0 z-50 flex flex-col bg-white dark:bg-slate-900 animate-in slide-in-from-bottom duration-300 md:max-w-md md:right-4 md:bottom-20 md:left-auto md:top-auto md:h-[600px] md:rounded-3xl md:shadow-2xl md:border md:border-slate-200">
          ...
          <form
            ...
            className="p-3 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex gap-2 shrink-0 md:rounded-b-3xl"
          >
            <input
            ...
            <button
              ...
              className="w-12 h-12 rounded-xl bg-secondary text-white flex items-center justify-center hover:bg-amber-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
