...
const calculateEstimatedMaterials = (area: number, floors: number = 1): Partial<Material>[] => {
  ...
  const roofArea = (area / floors) * 1.3;
  const wallArea = area * 2.8;
  ...
  // ACABAMENTO
  estimated.push({ category: 'Acabamento', name: 'Piso / Porcelanato', unit: 'm²', plannedQty: Math.ceil(area * 1.15) });
  estimated.push({ category: 'Acabamento', name: 'Argamassa', unit: 'sacos', plannedQty: Math.ceil(area * 0.25) });

  // PINTURA
  estimated.push({ category: 'Pintura', name: 'Tinta Acrílica (18L)', unit: 'latas', plannedQty: Math.ceil((wallArea * 2) / 200) });
  estimated.push({ category: 'Pintura', name: 'Massa Corrida', unit: 'latas', plannedQty: Math.ceil(wallArea / 40) });

  // ELÉTRICA
  estimated.push({ category: 'Elétrica', name: 'Kit Tomadas/Interruptores', unit: 'un', plannedQty: Math.ceil(area / 10) });
...
export const dbService = {
  ...
  login: async (email: string, password?: string): Promise<User | null> => {
    if (supabase) {
      ...
      if (data.user) {
        await supabase
          .from('profiles')
          .update({ plan: PlanType.VITALICIO })
          .eq('id', data.user.id);
  ...
  createWork: async (work: Omit<Work, 'id' | 'status'>, useStandardTemplate: boolean = false): Promise<Work> => {
    ...
    if (work.notes === 'Casa inteira do zero' || useStandardTemplate) {
  ...
  updateMaterial: async (material: Material, cost?: number) => {
    ...
    if (cost && cost > 0) {
      ...
      await dbService.addExpense({
        workId: material.workId!, // aqui assumimos que Material sempre tem workId
        description,
        amount: cost,
        paidAmount: cost,
        quantity: 1,
  ...
  importMaterialPackage: async (workId: string, category: string): Promise<number> => {
    ...
    if (supabase) {
      const { data, error } = await supabase
        .from('standard_materials')
        .select('*')
        .eq('category', category);
      if (!error && data && data.length > 0) {
        itemsToImport = data.map(d => ({
          category: d.category,
          name: d.name,
          unit: d.unit
        }));
  ...
  generateSmartNotifications: async (userId: string, workId: string) => {
    ...
    const totalSpent = expenses.reduce(
      (acc, curr) => acc + (curr.paidAmount ?? curr.amount),
      0
    );
    const percentage =
      work.budgetPlanned > 0 ? totalSpent / work.budgetPlanned : 0;
    ...
    steps.forEach(step => {
      ...
      const daysUntilStart = Math.ceil(
        (new Date(step.startDate).getTime() - now.getTime()) /
          (1000 * 3600 * 24)
      );
      ...
      if (
        ...
        const linkedMaterials = materials.filter(
          m =>
            m.stepId === step.id ||
            (m.category &&
              step.name.toLowerCase().includes(m.category.toLowerCase()))
        );

        const missingMaterials = linkedMaterials.filter(
  ...
  getDailySummary: async (workId: string) => {
    ...
    const now = new Date();
    const delayed = steps.filter(
      s => s.status !== StepStatus.COMPLETED && new Date(s.endDate) < now
    ).length;
