...
export const dbService = {
  ...
  async addStep(step: Omit<Step, 'id' | 'status' | 'orderIndex'>): Promise<Step> { 
    ...
    if (step.startDate && step.endDate && new Date(step.endDate) < new Date(step.startDate)) {
      throw new Error("A data de término não pode ser anterior à data de início.");
    }
    if (!step.startDate && step.endDate) {
      throw new Error("A data de término não pode ser definida se a data de início não estiver definida.");
    }
    ...
    const dbStep = {
      work_id: step.workId,
      name: step.name,
      start_date: step.startDate || null, // Allow null
      end_date: step.endDate || null,     // Allow null
      real_date: null, // Always null on creation
  ...
  async updateStep(step: Step): Promise<Step> {
    ...
    // realDate definida quando startDate for null
    if (step.realDate && !step.startDate) {
      throw new Error("A data de conclusão não pode ser definida se a data de início não estiver definida.");
    }
    // realDate < startDate
    if (step.realDate && step.startDate && new Date(step.realDate) < new Date(step.startDate)) {
    ...
    if (step.realDate && !currentDbStep.real_date) { // If realDate is being set (transitioning to COMPLETED)
      const { data: checklists, error: checklistError } = await supabase
        .from('checklists')
        .select('items')
        .eq('work_id', step.workId);

      if (checklistError) {
        console.error("Erro ao buscar checklists para validação:", checklistError);
        throw new Error("Erro ao validar checklists para conclusão da etapa.");
      }

      const hasPendingChecklistItems = (checklists || []).some(checklist =>
        checklist.items.some((item: ChecklistItem) => !item.checked)
      );

      if (hasPendingChecklistItems) {
        throw new Error("Não é possível concluir a etapa. Existem itens pendentes em checklists associadas a esta obra.");
      }
    }

    const dbStepUpdates: any = { // Use any to allow dynamic property assignment based on nullable types
      name: step.name,
      start_date: step.startDate || null,
      end_date: step.endDate || null,
      real_date: step.realDate || null, // Explicitly set to null if undefined
      // status and is_delayed are derived, not updated directly, so don't include here
