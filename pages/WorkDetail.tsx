...
const ContactsView: React.FC<{ mode: 'TEAM' | 'SUPPLIERS'; onBack: () => void }> = ({ mode, onBack }) => {
  ...
  const handleDeleteClick = (id: string) => {
    setZeModal({
      ...
      message: `Tem certeza que quer apagar este contato da sua lista de ${
        mode === 'TEAM' ? 'Equipe' : 'Fornecedores'
      }?`,
      onConfirm: async () => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
            <h3 className="text-lg font-bold mb-4 dark:text-white">
              Novo {mode === 'TEAM' ? 'Membro' : 'Fornecedor'}
            </h3>
            <form onSubmit={handleAdd} className="space-y-3">
              <input
                ...
                className="w-full p-3 rounded-xl border dark:border-slate-700 dark:bg-slate-800 dark:text-white outline-none focus:ring-2 focus:ring-primary"
                required
              />
              <input
                placeholder={
                  mode === 'TEAM' ? 'Profissão (ex: Pedreiro)' : 'Categoria (ex: Elétrica)'
                }
                value={newRole}
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-3 bg-primary text-white rounded-xl font-bold hover:bg-slate-800 transition-colors"
                >
                  Salvar
                </button>
              </div>
      ...
      <ZeModal
        ...
        onCancel={() =>
          setZeModal({ isOpen: false, title: '', message: '', onConfirm: () => {} })
        }
      />
    </div>
  );
...
const PhotosView: React.FC<{ workId: string; onBack: () => void }> = ({ workId, onBack }) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4">
      ...
      {photos.length === 0 && (
        <p className="text-center text-slate-400 py-10">Nenhuma foto adicionada.</p>
      )}
    </div>
  );
};
...
const ReportsView: React.FC<{ workId: string; onBack: () => void }> = ({
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4">
      <button
        onClick={onBack}
        className="mb-4 text-sm font-bold text-slate-400 hover:text-primary print:hidden"
      >
        <i className="fa-solid fa-arrow-left" /> Voltar
      </button>
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={handlePrint}
          className="bg-slate-100 text-primary w-10 h-10 rounded-xl flex items-center justify-center shadow-sm print:hidden"
        >
          ...
          <tbody>
            {expenses.map((e) => (
              ...
              >
                ...
                <td className="py-3 font-medium dark:text-slate-300">
                  {e.description}
                </td>
                <td className="py-3 text-right font-bold dark:text-white">
...
const StepsTab: React.FC<{ workId: string; refreshWork: () => void }> = ({
  ...
  const toggleStatus = async (step: Step) => {
    ...
    if (
      ...
      const matchPkg = FULL_MATERIAL_PACKAGES.find((p) =>
        step.name.toLowerCase().includes(p.category.toLowerCase()),
      );
      if (matchPkg) {
  ...
  const handleConfirmImport = async () => {
    if (pendingStartStep && foundPackage) {
      ...
      if (count > 0)
        alert(`${count} materiais sugeridos foram adicionados!`);
    }
  };
  ...
  const handleUpdateStep = async (e: React.FormEvent) => {
    ...
    if (editingStep) {
      ...
      const isStarting =
        originalStep &&
        originalStep.status === StepStatus.NOT_STARTED &&
        editingStep.status === StepStatus.IN_PROGRESS;

      if (isStarting) {
        const matchPkg = FULL_MATERIAL_PACKAGES.find((p) =>
          editingStep.name.toLowerCase().includes(p.category.toLowerCase()),
        );
        if (matchPkg) {
      ...
      <div className="space-y-4">
        ...
        {steps.length === 0 && (
          <div className="text-center py-12 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
            <i className="fa-solid fa-list-check text-4xl text-slate-200 dark:text-slate-700 mb-3" />
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm p-8 shadow-2xl animate-in zoom-in-95">
            ...
            <form onSubmit={handleCreateStep} className="space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 py-4 rounded-xl bg-primary text-white font-bold hover:bg-slate-800 transition-colors shadow-lg"
                >
                  Salvar
                </button>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm p-6 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex gap-5 mb-6">
                ...
                <div>
                  <h3 className="text-xl font-bold text-primary dark:text-white leading-tight mb-1">
                    Oi, chefe!
                  </h3>
                  <p className="text-sm text-slate-500">
                    Começando a <strong>{foundPackage}</strong>?
                  </p>
                </div>
              </div>
              ...
              <div className="mb-6 bg-slate-50 dark:bg-slate-800 p-5 rounded-2xl text-sm text-slate-600 dark:text-slate-300 leading-relaxed border border-slate-100 dark:border-slate-700">
                <p>
                  Posso adicionar a <strong>lista de materiais padrão</strong> dessa
                  fase pra você agora mesmo.
                </p>
                <p className="mt-2 text-xs font-bold text-secondary uppercase tracking-wide">
                  Economia de tempo: ~10 minutos
                </p>
              </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  <i className="fa-solid fa-wand-magic-sparkles" /> Gerar Lista Automática
                </button>
                <button
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-20">
      <div className="flex items-center justify-between mb-8">
        <SectionHeader
          title="Materiais"
          subtitle="Controle de compras e estoque."
        />
        <div className="flex gap-2">
          <button
          ...
          <div key={cat} className="mb-8 last:mb-0">
            ...
            <div className="space-y-3">
              {groupedMaterials[cat].map((m) => {
                ...
                const percentage =
                  Math.min(100, Math.round((m.purchasedQty / m.plannedQty) * 100)) || 0;
                return (
                  <div
                    key={m.id}
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase ${
                          isPurchased ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'
                        }`}
                      >
                        ...
                        <div className="h-1.5 w-full bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full ${
                              isPurchased ? 'bg-success' : 'bg-secondary'
                            }`}
      ...
      {materials.length === 0 && (
        <div className="text-center py-12 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
          <i className="fa-solid fa-cart-shopping text-4xl text-slate-200 dark:text-slate-700 mb-3" />
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setNewMaterial({ ...newMaterial, plannedQty: e.target.value })
                  }
                />
                <input
                  placeholder="Un (kg, m, un)"
                  className="w-full px-4 py-3 rounded-xl border bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 outline-none"
              ...
              <input
                placeholder="Categoria (ex: Pintura)"
                className="w-full px-4 py-3 rounded-xl border bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700 outline-none"
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="number"
                    className="w-full pl-8 pr-4 py-2 rounded-xl border bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700"
                    placeholder="0.00"
                    value={editCost}
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Se preencher, lança no financeiro automaticamente.
                </p>
              </div>
              ...
              {FULL_MATERIAL_PACKAGES.map((pkg) => (
                ...
                >
                  ...
                  <p className="text-xs text-slate-400">
                    {pkg.items.length} itens sugeridos
                  </p>
                </button>
              ))}
            </div>
...
const ExpensesTab: React.FC<{ workId: string; onUpdate: () => void }> = ({
  ...
  // Auto-fill description for Labor (placeholder for future logic)
  useEffect(() => {
    if (formData.category === ExpenseCategory.LABOR && !formData.description) {
      // Optional: lógica futura pra auto-preencher descrição
    }
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-20">
            ...
            <div className="space-y-3">
              {groupedExpenses[group].items.map((expense) => {
                ...
                const isPartial =
                  ...
                  (expense.paidAmount || 0) < expense.amount;
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs ${
                            expense.category === ExpenseCategory.MATERIAL
      ...
      {expenses.length === 0 && (
        <div className="text-center py-12 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl">
          <i className="fa-solid fa-wallet text-4xl text-slate-200 dark:text-slate-700 mb-3" />
...
const MoreMenuTab: React.FC<{ workId: string }> = ({ workId }) => {
  ...
  if (activeSection === 'TEAM')
    return (
      <ContactsView mode="TEAM" onBack={() => setActiveSection(null)} />
    );
  ...
  if (activeSection === 'REPORTS')
    return <ReportsView workId={workId} onBack={() => setActiveSection(null)} />;
  if (activeSection === 'AI') {
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
          ...
          <p className="text-slate-500 mb-6">
            Seu assistente está disponível no ícone de robô no topo da tela em qualquer lugar
            do app.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-24">
      <SectionHeader
        title="Mais Opções"
        subtitle="Gestão completa e ferramentas."
      />
            ...
            {aiHistory.length === 0 && (
              <div className="h-full flex flex-col items-center justify-center text-center opacity-40 p-6">
                ...
                <p className="text-sm font-medium">
                  "Fala chefe! Tô aqui pra ajudar. Pode perguntar sobre a obra, materiais ou
                  pedir uma dica!"
                </p>
              </div>
            )}
