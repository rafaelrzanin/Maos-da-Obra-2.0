...
const WorkDetail: React.FC = () => {
    ...
    const [expenseModal, setExpenseModal] = useState<{ isOpen: boolean, mode: 'ADD'|'EDIT', id?: string, isAutoGenerated?: boolean }>({ isOpen: false, mode: 'ADD', isAutoGenerated: false });
    ...
    const openAddExpense = () => {
        setExpenseModal({ isOpen: true, mode: 'ADD', isAutoGenerated: false });
        setExpDesc('');
        setExpAmount('');
        setExpTotalAgreed('');
    ...
    const openEditExpense = (expense: Expense) => {
        // Detect if this is an auto-generated expense linked to a material
        const isAuto = !!expense.relatedMaterialId;
    ...
    const handleSaveExpense = async (e: React.FormEvent) => {
        ...
        // Block save for auto-generated entries if they are in Read-Only mode
        if (expenseModal.isAutoGenerated) {
            return;
        }
        ...
        setExpenseModal({ isOpen: false, mode: 'ADD', isAutoGenerated: false });
        await load();
    };
                    ...
                    {filteredSteps.map((step) => {
                        ...
                        return (
                            <div key={step.id} className="mb-8 rounded-3xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                                ...
                                <div className="p-4 bg-slate-200 dark:bg-slate-800 border-b border-slate-300 dark:border-slate-700 flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-700 text-secondary font-black text-lg flex items-center justify-center shadow-sm">
                                        {String(originalIdx+1).padStart(2,'0')}
                                    </div>
                                    <h3 className="font-black text-lg text-primary dark:text-white uppercase tracking-tight">{step.name}</h3>
                                </div>

                                <div className="p-4 space-y-3">
                                    {stepMaterials.length === 0 ? (
                                        <div className="text-center py-6 text-slate-400 text-sm font-medium border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-2xl">
                                        ...
                                        stepMaterials.map(mat => {
                                            ...
                                            return (
                                                <div key={mat.id} onClick={() => { setMaterialModal({isOpen: true, material: mat}); setMatName(mat.name); setMatBrand(mat.brand||''); setMatPlannedQty(String(mat.plannedQty)); setMatUnit(mat.unit); setMatBuyQty(''); setMatBuyCost(''); }} className={`bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border shadow-sm cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-md ${statusText === 'Concluído' ? 'border-green-200 dark:border-green-900/30' : 'border-slate-200 dark:border-slate-700'}`}>
                                                    <div className="flex justify-between items-start mb-2">
                                                    ...
                                                    <div className="mt-3 flex items-center gap-3">
                                                        <div className="flex-1 h-2.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden"><div className={`h-full transition-all duration-500 ${barColor}`} style={{ width: `${progress}%` }}></div></div>
                                                        <div className="text-xs font-mono font-bold text-slate-500 whitespace-nowrap bg-white dark:bg-slate-800 px-2 py-1 rounded border border-slate-100 dark:border-slate-700">{mat.purchasedQty}/{mat.plannedQty} {mat.unit}</div>
                                                    </div>
                                                </div>
                                            );
                                        })
        ...
        if (activeTab === 'FINANCIAL') {
            return (
                <div className="space-y-6 animate-in fade-in">
                    ...
                    {/* Grouped Financial Expenses by Step -> Description (Product/Service) */}
                    {[...steps, { id: 'general', name: 'Despesas Gerais / Sem Etapa', startDate: '', endDate: '', status: StepStatus.NOT_STARTED, workId: '', isDelayed: false }].map((step, idx) => {
                        ...
                        stepExpenses.forEach(exp => {
                            ...
                            if (!groupedByDescription[key]) groupedByDescription[key] = [];
                        ...
                        return (
                            <div key={step.id} className="mb-8 rounded-3xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
                                {/* DISTINCT STEP HEADER */}
                                <div className={`p-4 border-b border-slate-300 dark:border-slate-700 flex items-center gap-3 ${isGeneral ? 'bg-slate-100 dark:bg-slate-800' : 'bg-slate-200 dark:bg-slate-800'}`}>
                                    {!isGeneral && <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-700 text-secondary font-black text-lg flex items-center justify-center shadow-sm">{String(idx + 1).padStart(2, '0')}</div>}
                                    <h3 className={`font-black text-lg uppercase tracking-tight ${isGeneral ? 'text-slate-500' : 'text-primary dark:text-white'}`}>{isGeneral ? step.name : step.name}</h3>
                                </div>
                                
                                <div className="p-4 space-y-6">
                                    {Object.keys(groupedByDescription).map(descKey => {
                                        const groupExpenses = groupedByDescription[descKey];
                                        return (
                                            <div key={descKey} className="space-y-2">
                                                {/* PRODUCT / SERVICE SUB-HEADER */}
                                                <div className="flex items-center gap-2 px-2 pb-1 border-b border-slate-100 dark:border-slate-800">
                                                    <div className="w-2 h-2 rounded-full bg-secondary"></div>
                                                    <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 uppercase tracking-wide">{descKey}</h4>
                                                </div>
                                                ...
                                                {groupExpenses.map(exp => {
                                                    ...
                                                    return (
                                                        <div 
                                                            ...
                                                            className="relative group bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:shadow-md cursor-pointer hover:border-secondary/30"
                                                        >
                                                            <div className="flex items-center justify-between mb-2">
                                                                <div className="flex items-center gap-4">
                                                                    ...
                                                                    <div>
                                                                        <p className="font-bold text-primary dark:text-white text-base leading-tight">Pagamento</p>
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm p-6 shadow-2xl overflow-hidden">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-bold text-primary dark:text-white">
                                {expenseModal.mode === 'ADD' ? 'Novo Gasto' : (expenseModal.isAutoGenerated ? 'Visualizar Gasto' : 'Editar Gasto')}
                            </h3>
                            
                            {/* Only show delete if NOT auto-generated */}
                            {expenseModal.mode === 'EDIT' && !expenseModal.isAutoGenerated && (
                        ...
                        {expenseModal.isAutoGenerated && (
                            <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900 rounded-xl flex gap-3 text-xs text-amber-700 dark:text-amber-400 font-medium">
                                <i className="fa-solid fa-lock mt-0.5"></i>
                                <p>Este gasto foi gerado automaticamente pela compra de materiais. Para alterar, vá na aba Materiais.</p>
                            </div>
                        )}
                        ...
                        <form onSubmit={handleSaveExpense} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    disabled={expenseModal.isAutoGenerated}
                                    className="w-full p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 font-bold text-primary dark:text-white focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all disabled:opacity-70 disabled:cursor-not-allowed" 
                                    required 
                                />
                            </div>
                            ...
                            <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input 
                                                ...
                                                disabled={expenseModal.isAutoGenerated} 
                                                className="w-full pl-9 p-2.5 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 font-bold text-sm disabled:opacity-70" 
                                            />
                                        </div>
                                    </div>
                                ...
                                <div>
                                    <label className="text-[10px] font-black text-green-600 dark:text-green-400 uppercase tracking-widest ml-1">
                                        {expenseModal.isAutoGenerated ? 'Valor Desta Compra' : 'Pagar Agora / Esta Parcela'}
                                    </label>
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            disabled={expenseModal.isAutoGenerated}
                                            className="w-full pl-10 p-3 bg-green-50 dark:bg-green-900/10 rounded-xl border-2 border-green-200 dark:border-green-800 font-black text-xl text-green-700 dark:text-green-400 focus:outline-none focus:border-green-500 disabled:opacity-70" 
                                            required 
                                        />
                                    </div>
                                </div>
                                ...
                                {expTotalAgreed && Number(expTotalAgreed) > 0 && !expenseModal.isAutoGenerated && (
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <select disabled={expenseModal.isAutoGenerated} value={expCategory} onChange={e => setExpCategory(e.target.value)} className="w-full p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-bold disabled:opacity-70">
                                ...
                                <div>
                                    ...
                                    <input type="date" disabled={expenseModal.isAutoGenerated} value={expDate} onChange={e => setExpDate(e.target.value)} className="w-full p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-bold disabled:opacity-70" required />
                                </div>
                            </div>
                            
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Etapa Relacionada</label>
                                <select disabled={expenseModal.isAutoGenerated} value={expStepId} onChange={e => setExpStepId(e.target.value)} className="w-full p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 font-bold text-sm disabled:opacity-70">
                            ...
                            <div className="flex gap-2 pt-2">
                                <button type="button" onClick={() => setExpenseModal({isOpen: false, mode: 'ADD', isAutoGenerated: false})} className="flex-1 bg-slate-100 dark:bg-slate-800 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                                    {expenseModal.isAutoGenerated ? 'Fechar' : 'Cancelar'}
                                </button>
