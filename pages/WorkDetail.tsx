...
const WorkDetail: React.FC = () => {
    ...
    const load = async () => {
        ...
        if (w) {
            ...
            setSteps(s ? s.sort((a: Step, b: Step) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime()) : []);
            setMaterials(m || []);
            setExpenses(e ? e.sort((a: Expense, b: Expense) => new Date(b.date).getTime() - new Date(a.date).getTime()) : []);
            setWorkers(wk || []);
            setSuppliers(sp || []);
            setPhotos(ph || []);
            setFiles(fl || []);
        ...
        <div className="max-w-4xl mx-auto min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col font-sans relative">
            <div className="bg-white dark:bg-slate-900 px-6 pt-6 pb-2 sticky top-0 z-20 shadow-sm border-b border-slate-100 dark:border-slate-800 no-print">
                <div className="flex justify-between items-center mb-1">
                    ...
                    {subView === 'NONE' && (
                        <div className="hidden md:flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl shrink-0 gap-1 absolute right-6">
                            {[
            ...
            {/* MAIN NAVIGATION BOTTOM BAR (Only visible on main tabs, mobile) */}
            {subView === 'NONE' && (
