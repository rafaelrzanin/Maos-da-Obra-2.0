...
import { StepStatus, FileCategory, ExpenseCategory, ExpenseStatus, type Work, type Worker, type Supplier, type Material, type Step, type Expense, type WorkPhoto, type WorkFile, type Contract, type Checklist, PlanType } from '../types.ts';
import { STANDARD_JOB_ROLES, STANDARD_SUPPLIER_CATEGORIES, CONTRACT_TEMPLATES, CHECKLIST_TEMPLATES } from '../services/standards.ts';
    ...
    if (entityType === 'step') {
        ...
        switch (step.status) {
            ...
            case StepStatus.DELAYED:
                ...
                break;
            default:
                statusText = 'Pendente'; icon = 'fa-hourglass-start';
        }
        ...
        switch (expense.status) {
            ...
            case ExpenseStatus.OVERPAID:
                statusText = 'Excedente'; bgColor = 'bg-purple-500'; icon = 'fa-arrow-up';
                break;
            default:
                statusText = 'Pendente'; icon = 'fa-clock';
        }
    }
...
const ToolCard: React.FC<ToolCardProps> = ({ icon, title, description, onClick, isLocked, requiresVitalicio }) => (
    ...
    >
        {isLocked && <div className="absolute inset-0 bg-black/10 rounded-2xl flex items-center justify-center z-10"><i className="fa-solid fa-lock text-primary text-2xl"></i></div>}
        <div className="w-12 h-12 rounded-full bg-secondary/10 text-secondary flex items-center justify-center text-xl shrink-0"><i className={`fa-solid ${icon}`}></i></div>
        <h3 className="font-bold text-primary dark:text-white leading-tight">{title}</h3>
        <p className="text-sm text-slate-500 dark:text-slate-400">{description}</p>
        {requiresVitalicio && <span className="text-[10px] font-bold text-amber-600 uppercase tracking-widest mt-auto"><i className="fa-solid fa-crown mr-1"></i> Vitalício</span>}
    </button>
);
...
const ToolSubViewHeader: React.FC<ToolSubViewHeaderProps> = ({ title, onBack, onAdd }) => (
    <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
            <button onClick={onBack} className="text-slate-400 hover:text-primary transition-colors"><i className="fa-solid fa-arrow-left text-xl"></i></button>
            <h2 className="text-2xl font-black text-primary dark:text-white">{title}</h2>
        </div>
        {onAdd && <button onClick={onAdd} className="px-4 py-2 bg-secondary text-white text-sm font-bold rounded-xl flex items-center gap-2"><i className="fa-solid fa-plus"></i> Novo</button>}
    </div>
);
...
const WorkDetail: React.FC<WorkDetailProps> = ({ activeTab, onTabChange }) => {
    ...
    const handleStepStatusChange = async (step: Step) => {
        ...
        const newRealDate = step.status === StepStatus.COMPLETED ? undefined : new Date().toISOString().split('T')[0];
        try {
    ...
    const handleAddStep = async (e: React.FormEvent) => {
        ...
        try {
            await dbService.addStep({ workId, name: newStepName, startDate: newStepStartDate, endDate: newStepEndDate, realDate: undefined });
    ...
    const handleEditStep = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setEditStepData(null); setShowAddStepModal(false);
            await loadWorkData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleAddMaterial = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setShowAddMaterialModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleEditMaterial = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setEditMaterialData(null); setShowAddMaterialModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };

    const handleDeleteMaterial = async (id: string) => {
        try { await dbService.deleteMaterial(id); await loadWorkData(); } catch (err) { console.error(err); }
    };
    ...
    const handleAddExpense = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setShowAddExpenseModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleEditExpense = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setEditExpenseData(null); setShowAddExpenseModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };

    const handleDeleteExpense = async (id: string) => {
        try { await dbService.deleteExpense(id); await loadWorkData(); } catch (err) { console.error(err); }
    };
    ...
    const handleAddPayment = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setShowAddPaymentModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleAddWorker = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setShowAddWorkerModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleEditWorker = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setEditWorkerData(null); setShowAddWorkerModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };

    const handleDeleteWorker = async (id: string) => {
        try { await dbService.deleteWorker(id, workId!); await loadWorkData(); } catch (err) { console.error(err); }
    };
    ...
    const handleAddSupplier = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setShowAddSupplierModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleEditSupplier = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setEditSupplierData(null); setShowAddSupplierModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };

    const handleDeleteSupplier = async (id: string) => {
        try { await dbService.deleteSupplier(id, workId!); await loadWorkData(); } catch (err) { console.error(err); }
    };
    ...
    const handleAddChecklist = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const items = newChecklistItems.filter(i => i.trim()).map(t => ({ id: `i-${Date.now()}-${Math.random()}`, text: t, checked: false }));
            await dbService.addChecklist({ workId: workId!, name: newChecklistName, category: newChecklistCategory, items });
            setShowAddChecklistModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleEditChecklist = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            const items = newChecklistItems.filter(i => i.trim()).map((t, idx) => ({ id: editChecklistData?.items[idx]?.id || `i-${Date.now()}`, text: t, checked: editChecklistData?.items[idx]?.checked || false }));
            await dbService.updateChecklist({ ...editChecklistData!, name: newChecklistName, category: newChecklistCategory, items });
            setEditChecklistData(null); setShowAddChecklistModal(false); await loadWorkData();
        } catch (err) { console.error(err); }
    };
    ...
    const handleToggleChecklistItem = async (clId: string, itemId: string) => {
        ...
        await dbService.updateChecklist({ ...cl, items }); await loadWorkData();
    };

    const handleDeleteChecklist = async (id: string) => {
        try { await dbService.deleteChecklist(id); await loadWorkData(); } catch (err) { console.error(err); }
    };

    const handleDragStart = (e: React.DragEvent, id: string) => { setDraggedStepId(id); e.dataTransfer.effectAllowed = 'move'; };
    const handleDragOver = (e: React.DragEvent, id: string) => { e.preventDefault(); setDragOverStepId(id); };
    ...
    if (loading || authLoading || !isUserAuthFinished || !work) {
        return <div className="flex items-center justify-center min-h-screen"><i className="fa-solid fa-circle-notch fa-spin text-3xl text-secondary"></i></div>;
    }
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {photos.map(p => (
                                <div key={p.id} className="relative group rounded-xl overflow-hidden shadow-lg">
                                    ...
                                    <button onClick={() => handleDeletePhoto(p.id, p.url)} className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full opacity-0 group-hover:opacity-100"><i className="fa-solid fa-trash"></i></button>
                                </div>
                            ))}
                        </div>
                    </>
    ...
    return (
        <div className="max-w-4xl mx-auto pb-12 pt-4 px-2 sm:px-4">
            {/* Header */}
            ...
            <div className={cx(surface, card, "animate-in fade-in")}>
                ...
                {activeTab === 'FERRAMENTAS' && renderSubViewContent()}
            </div>

            {/* Modals Implementation (Exemplo Simplicado para evitar erro de build) */}
            {showAddStepModal && (
                <ZeModal isOpen title="Etapa" confirmText="Salvar" onConfirm={editStepData ? handleEditStep : handleAddStep} onCancel={() => setShowAddStepModal(false)}>
                    <div className="space-y-4">
                        <input type="text" value={newStepName} onChange={e => setNewStepName(e.target.value)} placeholder="Nome" className="w-full p-3 rounded-xl border" />
                        <div className="grid grid-cols-2 gap-2">
                            <input type="date" value={newStepStartDate} onChange={e => setNewStepStartDate(e.target.value)} className="p-3 rounded-xl border" />
                            <input type="date" value={newStepEndDate} onChange={e => setNewStepEndDate(e.target.value)} className="p-3 rounded-xl border" />
                        </div>
                    </div>
                </ZeModal>
            )}
            ...
            {showAddExpenseModal && (
                <ZeModal isOpen title="Novo Gasto" confirmText="Adicionar" onConfirm={handleAddExpense} onCancel={() => setShowAddExpenseModal(false)}>
                    <div className="space-y-4">
                        <input type="text" value={newExpenseDescription} onChange={e => setNewExpenseDescription(e.target.value)} placeholder="Descrição" className="w-full p-3 rounded-xl border" />
                        <input type="number" value={newExpenseAmount} onChange={e => setNewExpenseAmount(e.target.value)} placeholder="Valor" className="w-full p-3 rounded-xl border" />
            ...
            {showAddPaymentModal && (
                <ZeModal isOpen title="Registrar Pagamento" confirmText="Pagar" onConfirm={handleAddPayment} onCancel={() => setShowAddPaymentModal(false)}>
                    <div className="space-y-4">
                        <p className="text-sm">Quanto você deseja pagar hoje?</p>
                        <input type="number" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} className="w-full p-3 rounded-xl border" />
...
// --- TYPES FOR MODALS (necessário para o TS não reclamar) ---
