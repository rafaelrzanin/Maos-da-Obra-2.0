                             ...
                             <div className="relative z-10">
                                 ...
                                 <div className="space-y-2">
                                     ...
                                     <div className="w-full bg-white/20 h-1.5 rounded-full overflow-hidden">
                                         <div className="h-full bg-orange-400" style={{width: `${totalMaterials > 0 ? ((materials.length - purchasedMaterials)/materials.length)*100 : 0}%`}}></div>
                                     </div>
                                 </div>
                             </div>
                         </div>
                             ...
                             <div key={cat} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 break-inside-avoid">
                                 ...
                                 <div className="grid grid-cols-1 gap-3">
                                     {groupedMaterials[cat].map(m => {
                                         const progress = m.plannedQty > 0
                                             ? Math.min(100, Math.round((m.purchasedQty / m.plannedQty) * 100))
                                             : 0;
...
const CalculatorView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    const renderContent = () => {
        if (mode === 'FLOOR') {
            ...
            return (
                <div className="animate-in fade-in slide-in-from-right-4">
                    <h3 className="text-lg font-bold text-primary dark:text-white mb-4">Pisos</h3>
        ...
        if (mode === 'WALL') {
            ...
            return (
                <div className="animate-in fade-in slide-in-from-right-4">
                    <h3 className="text-lg font-bold text-primary dark:text-white mb-4">Paredes</h3>
        ...
        if (mode === 'PAINT') {
            ...
            return (
                <div className="animate-in fade-in slide-in-from-right-4">
                    <h3 className="text-lg font-bold text-primary dark:text-white mb-4">Pintura</h3>
                    <div className="mb-6 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                        <label className="text-xs font-bold text-slate-500 uppercase">Área Parede (m²)</label>
                        <input
                            type="number"
        ...
        if (mode === 'ESTIMATOR') {
            ...
            return (
                <div className="animate-in fade-in slide-in-from-right-4">
                    <h3 className="text-lg font-bold text-primary dark:text-white mb-4">Estimativa</h3>
                    <div className="grid grid-cols-2 gap-4 mb-6">
                        <div>
                            <label className="text-xs font-bold text-slate-500 uppercase">Cômodos</label>
                            <input
                                type="number"
                    ...
                    <div className="space-y-4">
                        <ResultCard label="Tomadas/Interruptores" value={`${res.outlets + res.switches} un`} />
                        <ResultCard label="Pontos Hidráulicos" value={`${res.toilets + res.sinks} un`} />
                    </div>
                </div>
            );
        }
        ...
        return (
            <div className="animate-in fade-in slide-in-from-right-4 h-full flex flex-col">
                <button
                    ...
                    className="mb-4 text-sm font-bold text-slate-400 hover:text-primary flex items-center gap-2"
                >
                    <i className="fa-solid fa-arrow-left"></i> Voltar para Modelos
                </button>
                <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={handleCopy}
                            className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-200 transition-colors"
                        >
                            Copiar
                        </button>
                        <button
                            onClick={handleDownload}
                            className="bg-primary text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-primary-dark transition-colors flex items-center gap-2"
                        >
    ...
    if (activeSection === 'AI') {
        return (
            <div className="flex flex-col h-full">
                <button onClick={() => setActiveSection(null)} className="mb-4 text-sm font-bold text-slate-400 hover:text-primary">
                    <i className="fa-solid fa-arrow-left"></i> Voltar
                </button>
                <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
                    ...
                    <p className="text-slate-500 mb-6">
                        Seu assistente está disponível no ícone de robô no topo da tela em qualquer lugar do app.
                    </p>
                </div>
            </div>
        )
        ...
        <div className="flex items-center gap-4">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl shadow-sm ${
              stats.delayedSteps > 0 ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-600'
            }`}
          >
          ...
          <div>
            <h3
              className={`text-lg font-bold ${
                stats.delayedSteps > 0 ? 'text-red-600 dark:text-red-400' : 'text-primary dark:text-white'
              }`}
            >
              {stats.delayedSteps > 0
            ...
            >
              ...
              <div className="flex items-center gap-5 relative z-10">
                ...
                >
                  <i
                    className={`fa-solid ${
                      isComplete ? 'fa-check' : isInProgress ? 'fa-play text-[10px]' : isLate ? 'fa-exclamation' : 'fa-check'
                    }`}
                  ></i>
                </button>
                  ...
                  <div className="flex justify-between items-start">
                    <h4
                      className={`text-base font-bold mb-1 ${
                        isComplete ? 'line-through text-slate-400' : 'text-primary dark:text-white'
                      }`}
                    >
                      {step.name}
                    </h4>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditingStep({...editingStep, startDate: e.target.value})}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setEditingStep({...editingStep, endDate: e.target.value})}
                  />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
            ...
            {sortedCategories.map(cat => (
                <div key={cat} className="mb-8 last:mb-0">
                    ...
                    <div className="space-y-3">
                        {groupedMaterials[cat].map(m => {
                            const progress = m.plannedQty > 0
                                ? Math.min(100, (m.purchasedQty / m.plannedQty) * 100)
                                : 0;
                            return (
                                <div
                                    key={m.id}
                ...
                <div key={group} className="mb-8">
                    ...
                    <div className="space-y-3">
                        {groupedExpenses[group].items.map(expense => (
                            ...
                            >
                                <div className="flex justify_between items-start mb-2 flex justify-between items-start mb-2">
                                    <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-right">
                                        ...
                                        >
                                            {expense.paidAmount === expense.amount ? 'Pago' : 'Pendente'}
                                        </div>
                                    </div>
                                </div>
                            </div>
              ...
              <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50 dark:bg-black/20">
                {aiHistory.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-40 p-6">
                    ...
                    <p className="text-sm font-medium">"Fala chefe! Tô aqui pra ajudar."</p>
                  </div>
                )}
                {aiHistory.map((msg, i) => (
