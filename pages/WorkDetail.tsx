...
const WorkDetail: React.FC = () => {
    ...
    // Fix: Added isSubscriptionValid from useAuth context
    const { user, trialDaysRemaining, authLoading, isUserAuthFinished, isSubscriptionValid } = useAuth();
    ...
    // Step Modals & Forms - Fix: Declared missing state variables for the step modal
    ...
    const handleSaveStep = async (e: React.FormEvent) => {
        ...
        setStepName('');
        // Fix: Reset step dates after save
    ...
    // RESTAURADO: handleDeleteStep é agora chamado por um botão no card.
            ...
            return (
                <div className="space-y-4 animate-in fade-in pb-24 md:pb-0"> {/* Adjusted pb-24 for mobile, pb-0 for desktop */}
                    <div className="flex justify-between items-end mb-2 px-2">
                        ...
                        {/* Fix: Added correct state setters for step modal */}
                         ...
                         return (
                            <div key={step.id} className={`group bg-white dark:bg-slate-900 p-5 rounded-2xl border shadow-sm transition-all hover:shadow-md relative overflow-hidden ${cardBorderClass}`}>
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1 cursor-pointer flex flex-col sm:flex-row sm:items-center sm:justify-between" 
                                        // Fix: Added correct state setters for step modal when editing
                                        onClick={() => { setStepModalMode('EDIT'); setCurrentStepId(step.id); setStepName(step.name); setStepStart(step.startDate.split('T')[0]); setStepEnd(step.endDate.split('T')[0]); setIsStepModalOpen(true); }} 
            ...
            return (
                <div className="animate-in fade-in pb-24 md:pb-0">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        ...
                        <button onClick={() => setIsCalculatorModalOpen(true)} className={`p-4 rounded-xl flex flex-col items-center justify-center text-sm font-medium transition-colors ${isSubscriptionValid ? 'bg-slate-50 dark:bg-slate-800 hover:bg-secondary/10 dark:hover:bg-secondary/20 text-secondary border border-secondary/20' : 'bg-slate-200 dark:bg-slate-700 text-slate-400 cursor-not-allowed opacity-70 border border-slate-300 dark:border-slate-600'}`} disabled={!isSubscriptionValid} aria-label="Abrir calculadoras">
                            <i className="fa-solid fa-calculator text-2xl mb-2"></i> Calculadoras
                        </button>
                        <button onClick={() => setSubView('CONTRACTS')} className={`p-4 rounded-xl flex flex-col items-center justify-center text-sm font-medium transition-colors ${isSubscriptionValid ? 'bg-slate-50 dark:bg-slate-800 hover:bg-secondary/10 dark:hover:bg-secondary/20 text-secondary border border-secondary/20' : 'bg-slate-200 dark:bg-slate-700 text-slate-400 cursor-not-allowed opacity-70 border border-slate-300 dark:border-slate-600'}`} disabled={!isSubscriptionValid} aria-label="Ver modelos de contratos">
                            <i className="fa-solid fa-file-contract text-2xl mb-2"></i> Contratos
                        </button>
                        <button onClick={() => setSubView('CHECKLIST')} className={`p-4 rounded-xl flex flex-col items-center justify-center text-sm font-medium transition-colors ${isSubscriptionValid ? 'bg-slate-50 dark:bg-slate-800 hover:bg-secondary/10 dark:hover:bg-secondary/20 text-secondary border border-secondary/20' : 'bg-slate-200 dark:bg-slate-700 text-slate-400 cursor-not-allowed opacity-70 border border-slate-300 dark:border-slate-600'}`} disabled={!isSubscriptionValid} aria-label="Ver checklists de qualidade">
    ...
    const renderSubView = () => {
        // Performance fix: No need for pb-24 md:pb-0 on subviews themselves
        // The main container already has a reasonable pb-12.
        // If a subview needs more space, it should manage its own padding.
        if (subView === 'TEAM') {
            return (
                        ...
                        <form onSubmit={handleAddMaterial} className="space-y-4">
                            ...
                            {newMatBuyNow && (
                                <div className="grid grid-cols-2 gap-4 animate-in fade-in">
                                    <div>
                                        <label htmlFor="new-mat-buy-qty" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Qtd. Comprada AGORA</label>
                                    ...
                                    <div>
                                        <label htmlFor="new-mat-buy-cost" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Custo Total AGORA (R$)</label>
