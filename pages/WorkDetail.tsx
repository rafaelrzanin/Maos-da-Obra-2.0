...
const WorkDetail: React.FC = () => {
    ...
    // Ref to prevent initial useEffect race condition
    const skipEffectRef = useRef(false);
    
    // Ref to stabilize fetchPaymentHistory callback
    const expTotalAgreedRef = useRef(expTotalAgreed);
    useEffect(() => { expTotalAgreedRef.current = expTotalAgreed; }, [expTotalAgreed]);
    ...
    // --- FUNCTION: Centralized History Fetcher ---
    const fetchPaymentHistory = useCallback(async (description: string, currentExpenseId?: string) => {
        if (!work || description.trim().length <= 2) {
            setPaymentHistory({ totalPaid: 0, lastTotalAgreed: 0 });
            return;
        }
        
        const history = await dbService.getPaymentHistory(work.id, description, currentExpenseId);
        setPaymentHistory(history);
        
        // Auto-fill Total Agreed logic (only if not editing an existing value)
        // Using Ref here prevents this function from changing identity when expTotalAgreed changes
        if (currentExpenseId === undefined && !expTotalAgreedRef.current && history.lastTotalAgreed > 0) {
             setExpTotalAgreed(String(history.lastTotalAgreed));
        }
    }, [work]); // Removed expTotalAgreed from deps to stabilize

    // --- EFFECT: Debounced Fetch for Typing ---
    useEffect(() => {
        // Only run debounce if modal is open AND we are not in the initial skip phase
        if (expenseModal.isOpen) {
            ...
            const timer = setTimeout(() => {
                // Determine excludeId based on current modal mode
                const excludeId = expenseModal.mode === 'EDIT' ? expenseModal.id : undefined;
                fetchPaymentHistory(expDesc, excludeId);
            }, 500); 
            return () => clearTimeout(timer);
        }
    }, [expDesc, expenseModal.isOpen, expenseModal.mode, expenseModal.id, fetchPaymentHistory]);
    ...
    const openAddExpense = () => {
        setPaymentHistory({ totalPaid: 0, lastTotalAgreed: 0 }); // RESET
        ...
        skipEffectRef.current = true; // Skip first effect run
    };
    ...
    const openEditExpense = (expense: Expense) => {
        ...
        // RESET History immediately to prevent blinking old values
        setPaymentHistory({ totalPaid: 0, lastTotalAgreed: 0 });
        ...
        // Skip the effect for this initial open to avoid race condition
        skipEffectRef.current = true;
        
        // Fetch immediately with explicit ID exclusion
        fetchPaymentHistory(expense.description, expense.id);
    };
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-sm p-6 shadow-2xl">
                        ...
                        <form onSubmit={handleSaveExpense} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {/* 2. AMOUNT (Current Payment) */}
                                <div className="relative">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Valor Pago Agora</label>
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            disabled={expenseModal.isAutoGenerated}
                                            className="w-full pl-10 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 font-bold text-lg" 
                                            required 
                                        />
                                    </div>
                                </div>

                                {/* 3. TOTAL AGREED (Optional / Auto-filled) */}
                                <div className="relative">
                            ...
                            {/* 4. HISTORY DISPLAY (Dynamic) */}
                            {paymentHistory.totalPaid > 0 && (
                                <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-900 flex items-center gap-3 animate-in fade-in">
                                    ...
                                    <div>
                                        <p className="text-[10px] uppercase font-bold text-blue-400">Histórico Anterior</p>
                                        <p className="text-sm font-bold text-blue-700 dark:text-blue-200">
                                            Já pago: R$ {paymentHistory.totalPaid.toLocaleString('pt-BR')}
                                            {paymentHistory.lastTotalAgreed > 0 && (
                                                <span className="opacity-70 font-normal"> de R$ {paymentHistory.lastTotalAgreed.toLocaleString('pt-BR')}</span>
                                            )}
                                        </p>
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <select 
                                    value={expStepId} 
                                    onChange={e => setExpStepId(e.target.value)} 
                                    disabled={expenseModal.isAutoGenerated}
                                    className="w-full p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 font-bold text-sm"
                                >
                            ...
                            <div className="flex gap-2 pt-2">
                                <button type="button" onClick={() => setExpenseModal({isOpen: false, mode: 'ADD', isAutoGenerated: false})} className="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-500 py-3 rounded-xl font-bold">Cancelar</button>
                                {!expenseModal.isAutoGenerated && (
                                    <button type="submit" className="flex-1 bg-primary text-white py-3 rounded-xl font-bold">Salvar</button>
                                )}
                            </div>
