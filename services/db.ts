...
// --- ENGINE: CONSTRUCTION PLAN GENERATOR (ENGENHEIRO VIRTUAL 2.0 - MELHORADO) ---
interface PlanItem {
    stepName: string;
    duration: number;
...
const generateConstructionPlan = (totalArea: number, floors: number): PlanItem[] => {
    ...
    for (let i = 0; i < floors; i++) {
        ...
        // A. Alvenaria
        plan.push({
            stepName: `3.${i+1} Alvenaria (${floorLabel})`,
            duration: 20,
            startOffset: currentDay,
            materials: [
        ...
        // B. Laje
        plan.push({
            stepName: `3.${i+1} Laje (${floorLabel})`,
            duration: 15,
            startOffset: currentDay,
            materials: [
    ...
    // 4. TELHADO
    plan.push({
        stepName: "4. Telhado e Cobertura",
        duration: 15,
        startOffset: currentDay,
        materials: [
            ...
            { name: 'Caixa D\'água', unit: 'un', qty: 1 },
            { name: 'Manta Térmica', unit: 'rolos', qty: Math.ceil(footprint / 45) },
        ]
    });
    currentDay += 10; 

    // 5. INSTALAÇÕES GROSSAS
    plan.push({
        stepName: "5. Instalações (Tubulação e Caixinhas)",
        duration: 15,
        startOffset: currentDay,
        materials: [
            ...
            { name: 'Eletroduto Flexível', unit: 'rolos', qty: Math.ceil(totalArea / 20) },
            { name: 'Caixinhas 4x2 (Parede)', unit: 'un', qty: Math.ceil(totalArea / 8) },
    ...
    // 7. FIAÇÃO (Antes da pintura)
    plan.push({
        stepName: "7. Fiação e Cabos",
        duration: 7,
        startOffset: currentDay,
        materials: [
    ...
    // 8. PISOS
    plan.push({
        stepName: "8. Pisos e Revestimentos",
        duration: 20,
        startOffset: currentDay,
        materials: [
    ...
    plan.push({
        ...
        materials: [
            ...
            { name: 'Rolo e Pincel', unit: 'kit', qty: 1 },
        ]
    });
    currentDay += 15;
    ...
    plan.push({
        stepName: "10. Acabamentos Finais (Elét/Hid)",
        duration: 10,
        startOffset: currentDay,
        materials: [
...
export const dbService = {
    ...
    if (isConstructionMode) {
        ...
        for (const item of plan) {
            ...
            if (item.materials.length > 0) {
                 // IMPORTANTE: Forçar Categoria = Nome da Etapa para agrupar corretamente na visualização
                 const matPayloadRaw = item.materials.map(m => ({
                    name: m.name, planned_qty: m.qty, purchased_qty: 0, unit: m.unit,
                    category: item.stepName, // <-- AQUI ESTÁ A CORREÇÃO
                    step_id: stepId
                 }));
  ...
  importMaterialPackage: async (workId: string, category: string): Promise<number> => {
    ...
    // IMPORT FIX: Use step name if found, else category
    const catToUse = matchStep ? matchStep.name : category;
